...
**เว็บไซต์**: https://thering999.github.io/smiv_plus/ · **ประวัติการเปลี่ยนแปลง**: [CHANGELOG.md](CHANGELOG.md)

ทำงานทั้งหมดในเบราว์เซอร์ (vanilla JS + SheetJS + Chart.js + jsPDF/html2canvas, โฟลเดอร์ `docs/`) อ่านข้อมูลจาก `docs/data.json`
ซึ่งเป็นแหล่งข้อมูลกลางที่ทุกคนเห็นร่วมกัน — ไม่ต้องรัน server/DB ทิ้งไว้ ไม่มีค่าใช้จ่าย มี PWA (ใช้งานออฟไลน์ได้จากแคชล่าสุด)

**อัปโหลด Excel → เผยแพร่อัตโนมัติ**: อัปโหลดไฟล์แล้วเผยแพร่เข้า GitHub ทันทีโดยไม่ต้องกดอะไรเพิ่ม/ไม่ต้องใส่ token เลย
(ผ่าน Cloudflare Worker, `worker/index.js`, endpoint `https://smiv-plus-publish.habusaya.workers.dev` — ถือ GitHub token จริงไว้ฝั่งเซิร์ฟเวอร์
ไม่เคยอยู่ในโค้ดที่เผยแพร่ รายละเอียด/secret อยู่ใน `credentials.local.txt` ไม่ได้ commit เข้า git)
ทุกครั้งที่เผยแพร่ระบบเก็บสำเนาไว้เป็นประวัติที่ `docs/history/` ดู/กู้คืน/ลบย้อนหลังได้จากปุ่ม "📜 ประวัติการนำเข้า"

**สิทธิ์การเข้าถึง**: ด่านรหัสผ่าน 2 ระดับ — admin (แก้ไข/เผยแพร่/ลบได้) และ viewer (ดูอย่างเดียว) ดูรหัสได้ใน `credentials.local.txt`

**ทดสอบ**: `npm test` รันสูตรคำนวณ/ตรรกะแจ้งเตือน/ความถูกต้องของไฟล์ export (37 เคส, Node built-in test runner)
มี GitHub Actions รันอัตโนมัติทุกครั้งที่แก้ `docs/app.js`/`docs/ui.js`/`test/**`
...
- **ตัดผู้เสียชีวิตออกจากตัวนับ D/F แล้ว** — แต่เป็นการกรอกมือต่อปีงบ/อำเภอ (เมนู "ประชากร/ประมาณการณ์") เพราะระบบนี้ไม่มีช่องทางเชื่อมข้อมูลกระทรวงมหาดไทยจริงแบบอัตโนมัติเหมือน HDC ต้นทาง
...
- `การคัดกรองและติดตามดูแลประชาชนกลุ่มเสี่ยง...pdf` (จ.อุบลราชธานี) — เกณฑ์ 5 สัญญาณเตือน (SMI-V SCAN), 7 สัญญาณเตือนก่อความรุนแรง, คะแนน OAS/สี, หลัก "2ย1ส" — เป็นเกณฑ์คัดกรองระดับชุมชนของแอป V-Care (คนละระบบกับ smiv_plus ซึ่งใช้ข้อมูล HDC/43แฟ้ม) นำมาใส่ไว้ในแท็บ "คู่มือรหัส" เพื่อให้ผู้ใช้เห็นบริบทครบ
